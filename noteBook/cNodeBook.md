...
指针的表示方式是 [类型 加 *] （int*） 表示int类型的指针 地址
...
指针大小 在32位 是4字节 64位 是8字节

理由是什么呢 ！！
因为指针 存放的是地址 当32个二进制位  或者 64个二进制位 组成的 
所以 换算成 字节的话 指针变量的大小就是在32位系统中是 4字节 64位系统中 是 8字节
